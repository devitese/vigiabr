...
**Goal:** Port and adapt the max project's Claude Code pipeline to vigiaBR, giving the project a full development workflow from day one.

**Architecture:** Fork & Strip approach — copy max's `.claude/` infrastructure and `.github/workflows/`, then adapt all references from automotive CRM to Brazilian transparency platform. Commands route to skills, skills contain knowledge, subagents execute.
...
Replace the current CLAUDE.md with the routing trunk adapted from max. Key sections:
...
Merge the existing metadata-updater hook with the new hooks from max. The final settings.json should have:
...
Note: Removed `staging` branch references from max. Kept metadata-updater.sh as PostToolUse for Bash. Removed `pnpm format` from Stop (vigiaBR doesn't have pnpm yet — will add formatter when project has code).
...
Direct copy from max — no changes needed. This reads the using-superpowers SKILL.md and injects it at session start.

**Step 2: Create check-conflict-markers.sh**

Direct copy from max — scans for conflict markers before git commits. No changes needed.

**Step 3: Create ensure-docker-running.sh**

Direct copy from max — auto-starts Docker Desktop when docker commands are detected. No changes needed (vigiaBR uses Docker + Caddy).

**Step 4: Create check-python-syntax.sh**

Direct copy from max — validates Python files after Edit/Write. No changes needed.

**Step 5: Create quality-gate-task.sh**

Adapt from max:
- Change venv paths from `apps/ai/max-v3/.venv` and `apps/ai/webhooks/.venv` to vigiaBR-appropriate paths (search common locations: `.venv`, `venv`)
...
Direct copy from max — removes untracked CLAUDE.md files from wrong directories. No changes needed.
...
Adapt from max — same logic but with vigiaBR venv paths for ruff/mypy. Remove pnpm check-types, use tsc --noEmit with existence check.
...
Direct copy from max — the agent is generic enough to work as-is. No domain-specific references.
...
Adapt from max:
- Remove `eval`, `eval-dataset`, `max-test`, `test-auto` routes
...
Direct copy from max — generic pipeline health auditing. No domain-specific changes needed.
...
Adapt from max: same structure, remove Linear references from argument-hint. Change `DEA-###` to `#123`.
...
Direct copy from max — these are thin routers with no domain-specific content.
...
Adapt git.md: same structure as max but remove Linear references.
...
Direct copy from max — generic enough. Keep: verify tests → present 4 options → execute → clean up.
...
Direct copies from max — these are generic development workflow skills with no domain-specific content.
...
- Display dashboard format (same style as max)
...
Keep the parallel gate structure from max. Add Neo4j and PostgreSQL service containers for integration tests.
...
Direct copy from max — trigger on @claude mentions in issues/PRs. Generic workflow.
