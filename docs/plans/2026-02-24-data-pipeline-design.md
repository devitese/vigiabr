...
**Format:** JSONL files at `data/raw/{source}/YYYY-MM-DD/*.jsonl`

Each line is a JSON object conforming to `contracts/raw_formats/{source}.schema.json`. File-based contract — extraction writes, processing reads. No direct Python imports between them.
...
1. `uv run --project extraction scrapy crawl {spider_name}`
2. `uv run --project processing python -m processing.run {source}`
...
`platform/` is NOT a Python package — Docker + Airflow config files only.

---

## Per-Front Scope

### Front 1: `schemas/` — Database Schemas & Shared Types
...
### Front 2: `extraction/` — Scrapy Spiders & Bulk Downloaders
...
### Front 3: `processing/` — Transform, Validate, Load
...
### Front 4: `platform/` — Docker, Airflow, Monitoring
...
1. `contracts/` — JSON Schemas, DDL, data flow spec
2. Root `pyproject.toml` — uv workspace config
3. `.gitignore` — add `data/`, `.venv`
4. Empty directory scaffolds with placeholder `pyproject.toml` per front

### Parallel Execution

```
Pre-step ──┐
           ├── Worktree 1: schemas/     ──── PR #1 merges first ──┐
           ├── Worktree 2: extraction/  ──── codes against         ├── rebase on #1 → PR #2
           ├── Worktree 3: processing/  ──── contracts/ initially  ├── rebase on #1 → PR #3
           ├── Worktree 4: platform/    ──── Docker + Airflow      ├── PR #4 (no rebase needed)
```
